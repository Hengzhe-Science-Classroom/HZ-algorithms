...
const App = {
    ...
    // --- Progress ---
    loadProgress() {
        try { this.progress = JSON.parse(localStorage.getItem('algorithms-progress') || '{}'); }
        catch (e) { this.progress = {}; }
    },
    saveProgress() {
        try { localStorage.setItem('algorithms-progress', JSON.stringify(this.progress)); } catch (e) {}
    },
    ...
    getChapterProgress(ch) {
        ...
        const total = loaded ? loaded.sections.length : (done || 5);
        return { done, total };
    },
    getTotalProgress() {
        let done = 0, total = 0;
